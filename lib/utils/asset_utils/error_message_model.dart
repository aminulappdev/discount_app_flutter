class ErrorMessageModel {
  ...
  ErrorMessageModel.fromJson(Map<String, dynamic> json) {
    ...
    final sources = json['errorSources'] ?? json['errorSource'];
    if (sources is List) {
      errorSources = <ErrorSources>[];
      for (final source in sources) {
        if (source is Map<String, dynamic>) {
          errorSources!.add(ErrorSources.fromJson(source));
        }
      }
    }

    err = json['err'] is Map<String, dynamic> ? Err.fromJson(json['err']) : null;
    stack = json['stack'];
  }
